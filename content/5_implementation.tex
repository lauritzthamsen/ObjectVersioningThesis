...
When a proxy, however, traps write access and no version is available for the current system version, it creates a new version of the object before forwarding the write access.
...
The source transformations for introducing our proxy-based version-aware references wrap the original expressions into a \lstinline{proxyFor} function, which returns a proxy for an object.
...
In addition, UglifyJS supports Source Maps \todo{\url{https://docs.google.com/document/d/1U1RGAehQwRypUTovF1KRlpiOFze0b-_2gc6fAH0KY0k/edit\#heading=h.ue4jskhddao6}, accessed May 2, 2014}.
