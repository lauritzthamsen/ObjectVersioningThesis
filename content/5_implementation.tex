...
% content: limitations of our implementation that are not directly related to the state of the preliminary implementation of the proxies / for which we have no workaround .. except for the general limitation that these proxies have to be available, e.g. experimental javascript enabled in chrome


\subsubsection{Availabilty of ECMAScript 6 Proxies}

Our implementation requires the ECMAScript 6 Direct Proxies to be available.
These are, however, part of the next draft of ECMAScript, which has currently not been finalized, as described in \ref{sec:IMPLEMENTATION:4}.
...
Even if the Direct Proxies are available, the current implementation impedes debugging.
Proxies are partly implemented by a library and every trapped object access is visible in multiple frames in the debugger.
...
In addition, the development tools in Chrome do not handle proxies correctly under all circumstances.
...
\subsubsection{Concurrent JavaScript}
