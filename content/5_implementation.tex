...
\begin{enumerate}
    ...
    % \item When the prototype of an object is a proxy, it cannot be changed due to a bug in Chrome.
\end{enumerate}
...
% chrome bug was fixeeeeeeed :) 
% https://github.com/tvcutsem/harmony-reflect/issues/18
...
\todo{add something about when the versioning source transformations are done and what objects are versioned currently}
...
% limitations of our implementation that are not directly related to the state of the preliminary implementation of the proxies.. except for the general limitation that these proxies have to be available, e.g. experimental javascript enabled in chrome
