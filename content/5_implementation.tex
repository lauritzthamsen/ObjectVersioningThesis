...
\subsubsection{Proxies Trap and Forward Access}
...
When the system version changes, the next access to proxies returns the state of the system version for objects of the JavaScript runtime.
Certain JavaScript objects, however, provide only access to the HTML elements that the browser renders.
As these cannot be copied like usual JavaScript objects, we do not version these element objects.
The graphical state is, however, completeyl captured by the Lively Kernel's morphs, which are ordinary JavaScript objects and versioned.
Thus, when the system version changes, we update the HTML document from the current set of visible morphs in the new version.
