...
While these benchmarks do not test JavaScript's features systematically, they cover a range of language features that are representative of many JavaScript programs.
...
\paragraph{Method}
% what was done?
tested whether lively loads.. 

we tested the simple linear global undo/redo that we built for the Lively Kernel, as explained in \todo{reference..}, through various simple examples and with this, whether the source transformations and proxies together allow to preserve and re-establish versions of the Lively Kernel's JavaScript runtime in practice.

% why / to test what?

whether the version-aware references are also used consistently for all mutable objects and whether the version-aware references correctly delegate to particular versions.
...
% TODO: ++ lessons learned from letting the Lively Kernel run with our version-aware references


\paragraph{Discussion}

% Further, the linear global runtime versions allows to re-establish previous development states in the Lively Kernel.
