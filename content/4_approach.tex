...
This chapter introduces our approach to recovering previous system states in an object-oriented programming system.
The approach is based on alternative, version-aware references that manage versions of objects transparently.
For the Lively Kernel, we present a design that uses proxies for implementing the version-aware references.


\section{Version-aware References} \label{sec:APPROACH:1}

In different versions of a system such as the Lively Kernel, the objects have different states. 

\subsubsection{Versions of Objects}

For example, an object could represent an address.
The state of this \emph{address} object is shown in Figure~\ref{fig:SingleObject}.
...
If the object gets values assigned to its \lstinline{city} and \lstinline{number} fields, the object's state is changed.
As the address object's state is part of the system state, changing the address object changes the system state.
If we call the initial state version \emph{v1} and the state after making changes to the object version \emph{v2}, the state of the address object is different for the two versions of the system, as shown in Figure~\ref{fig:ObjectChanged}.
...
To be able to recover previous system states after making changes, the previous object states need to be accessible.
...
A version of an object is, in the simplest case, a full copy of an object.
...
There are now two versions of the \lstinline{address} objects in version \emph{v2}, as shown in Figure~\ref{fig:VersionPreserved}.
One of the objects holds the original state, while the other holds the state that the object should have in version \emph{v2} of the system.
The two objects hold no information that indicates to which version of the system they belong to.
...
That is, even after adding values to the fields of the \lstinline{address} object, the following statement would still return true when \lstinline{aPerson} refers to the \lstinline{person} object:
...
They can be assigned to variables, object fields, and get passed around.

When the \lstinline{person} object now holds a version-aware reference as its \lstinline{address} property, it can resolve the reference to the correct version.
...
\begin{figure}[h]
    ...
    \caption{A version-aware reference relates a \emph{person} object to both versions of its \emph{address} property.}
...
The previously presented \lstinline{person} object is here a \lstinline{company} object's \lstinline{CEO} property.
...
That is, only this version identifier has to be changed to have version-aware references resolve to other versions of objects.
...
Given the example situation from Figure~\ref{fig:ObjectGraphWithReferencesResolvedAlongVersion2} and given \lstinline{aCompany} refers to the \lstinline{company} object, the following statement would refer to three different values depending on the current version identifier:
...
The information that version \emph{v1} is the predecessor of version \emph{v2} can be used to resolve to the \emph{v1}-version of an object when no \emph{v2}-version is available.
...
It should, however, not be changed while multiple version-aware references of an object graph are transitively resolved.
That is, the version-aware references involved in evaluating the previous example statement should be resolved together using one version identifier.

To be able to actually re-establish a particular version of the system with our approach now, two requirements need to be fullfilled:
...
The version-aware references need to have access to the versions of objects that make up the particular system state.

With our approach, not every state is recoverable, but specific states have to be preserved.
...
This way, programmers can undo and redo the changes of specific actions, regardless of whether they preserved a version in anticipation of recovery needs or not.
...
Second, preserving a new version of the system also happens incrementally.
...
We used proxies to implement version-aware references in JavaScript for the Lively Kernel.
...
Given \lstinline{aPerson} refers to the \lstinline{person} object, evaluating the following code snippet needs to return \lstinline{true} in version \emph{v2}:
...
The proxies used for the implementation of this design are proxies that can intercept and forward read and write access.
Such proxies, also referred to as \emph{virtual objects}~\cite{VanCutsem2010PDP}, do not stand-in for specific objects, but can forward access to any object.
...
Ordinary references that usually refer to an object now need to refer to the proxy that stands-in for the object instead.

To use proxies consistently for all mutable objects, we create and return proxies for all new objects.
...
Proxies delegate to and create versions of an objet using a \emph{version of the system}.
...
Figure~\ref{fig:SystemVersions} shows an example with three system versions.
In the example, version \emph{v2} is the current version of the system.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.58\textwidth]{figures/4_approach/8_systemVersions.pdf}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.92\textwidth]{figures/4_approach/9_proxyUsesSystemVersion.pdf}
...
Therefore, a version of an object is changed only when it is the version for the current version of the system.

\emph{To re-establish a previous version}, the current system version has to be set to its predecessor.
In that case, proxies forward access to previous versions of the objects.

\emph{To preserve a version}, the current system version has to be set to another version.
...
In a new version \emph{v3} of the system, the proxy receives write access but has no version of its object for \emph{v3}.
It, therefore, copies the most recent version of the object and forwards the write access to the new version.
