...
Subsequentely, there are two \lstinline{address} objects in version \emph{v2}, as shown in Figure~\ref{fig:VersionPreserved}.
One of the two versions holds the original state, while the other holds the state that the object has in version \emph{v2} of the system.
...
For the single \lstinline{address} object, there are now two versions of the object.
These versions hold no information that indicates to which version of the system they belong to.
They also store no information that one object is a copy of the other one.
At the same time, references to objects remain unchanged after copying objects.
For example, there could have been a \lstinline{person} object with an \lstinline{address} property referring to the \lstinline{address} object.
...
A situation, in which a new version of an object has to be created, is shown in Figure~\ref{fig:NewVersion}.
